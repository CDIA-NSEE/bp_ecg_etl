    ...
    async with session.client('s3', region_name=config.AWS_REGION) as s3:
        try:
            ...
            logger.info("Successfully downloaded PDF", 
                       bucket=bucket, key=key, size=len(content))
            return content
            
        except ClientError as e:
            error_code = e.response.get('Error', {}).get('Code', 'Unknown')
            logger.error("Failed to download PDF", 
                        bucket=bucket, key=key, error=error_code)
            raise
...
def generate_output_key(input_key: str, suffix: str = "_anonymized") -> str:
    """Generate output key based on input key."""
    if '/' in input_key:
        path, filename = input_key.rsplit('/', 1)
        base_path = path + "/"
    else:
        filename = input_key
        base_path = ""
    ...
    if '.' in filename:
        ...
        return f"{base_path}{name}{suffix}.{ext}"
    else:
        return f"{base_path}{filename}{suffix}"
